...
public class AnalysisJsonMapper {
    ...
    public String writeImpactGraph(UsageGraphResponse impactGraph) {
        return impactGraph == null ? null : writeValue(impactGraph);
    }

    public UsageGraphResponse readImpactGraph(String json) {
        if (json == null || json.isBlank()) {
            return null;
        }
        return readValue(json, new TypeReference<>() {});
